...
frappe.ui.form.PrintView = class {
	...
	setup_toolbar() {
		...
		this.page.add_button(__("Enviar"), () => this.send_pdf(), {
			icon: "mail",
		});
	...
	add_sidebar_item(df, is_dynamic) {
		...
		if (df.fieldname === "letterhead" || df.fieldname === "language") {
	...
	show(frm) {
		...
		document.querySelectorAll(".custom-actions.hidden-xs.hidden-md").forEach(el => {
			console.log(el.innerHTML); // Muestra el contenido HTML dentro
		});
		...
		if (format == "Esterelización" && personaIngresante){
			...
			frappe.db.get_value("Persona", personaIngresante, "correo").then((respuesta) => {
				...
				if (respuesta.message) {
					let correo = respuesta.message.correo
					...
					frappe.msgprint({ message:("No hay un correo cargado"),
											indicator: 'red'})
				}
			})
		}
