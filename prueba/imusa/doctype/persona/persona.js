...
frappe.ui.form.on('Persona', {
    ...
    after_save: function(frm){
        ...
        if (frm.custom_opciones){
            ...
            if (parametros !== null){
                frappe.call({
                    method: "prueba.imusa.api.insertar_direccion", 
                    args: parametros,
                })
            }
        }
...
function load_persona(frm){
    frappe.call({  
        method: "prueba.imusa.api.get_persona_data", 
        args: { 
...
function show_dirreccion(frm){
    frappe.db.get_value('Direcciones', frm.doc.direccion, fields=['coordenada_x', 'coordenada_y']).then((s) => {s
        frappe.call({
            method: "prueba.imusa.api.conversion",
...
function options_handler(frm){
    ...
    frappe.call({
        method: "prueba.imusa.api.get_features", 
